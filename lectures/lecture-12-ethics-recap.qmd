...
::: {.callout-warning}
The footprint of *one* query is small. The footprint of these systems *existing at scale* is not — and it depends on **where** data centres are sited. Moving to renewable-rich regions helps, but is only a partial fix. This is a policy question, not a technical one.
:::
...
::: {.card}
Egami et al. (2023) [@egami2023imperfect] show the sharper version: feeding LLM labels straight into a downstream regression gives **biased coefficients and confidence intervals that don't cover** — at 80–90% accuracy, the range where a researcher would call the labels fine. The bias comes from the errors being *correlated with the thing you are studying*.
:::
...
* The cost is the subset — a few hundred documents, coded properly.
:::
:::

::: {.callout-tip}
This is the honest answer to "can I use an LLM to code my data?" Yes — as long as you also hand-code enough of it to find out how wrong the model was.
:::

## How do you test for harm?

[Capability evaluation, from the other direction]{.eyebrow}

Accuracy asks *how often is it right*. Safety evaluation asks *what is the worst thing it will do if someone tries*.

::: {.columns}
::: {.column width="50%"}
[Red-teaming]{.pill-amber}

People — and now models — attack the system to make it fail.

The UK AI Safety Institute tested five publicly deployed LLMs in **May 2024**: all were highly vulnerable to **basic** jailbreaks, and some produced harmful outputs with no jailbreak attempt at all.
:::
::: {.column width="50%"}
[Dangerous-capability evals]{.pill-amber}

Can it assist a cyber-attack, or with chemical and biological weapons? Can it act on its own over long horizons?

**Human-uplift studies** ask the harder question: does the model make a non-expert *more* capable than a search engine would? That is the counterfactual that matters.
:::
:::

::: {.card}
Run by the labs themselves — Anthropic's Responsible Scaling Policy, OpenAI's Preparedness Framework, Google DeepMind's Frontier Safety Framework — and by third parties. We come back to who watches the watchers shortly.
:::

::: {.callout-warning}
A failed elicitation is not proof of absence. It shows *this* attempt didn't find the capability. Someone with more time, better prompts, or fine-tuning access may.
:::
...
::: {.callout-warning}
**The honest position**: we are early. Aggregate labour-market data lags by 1–3 years. Decisions are being made now — about policy, training, social safety nets — on weak evidence. Whether you build these systems or use them, you have a stake in **how the gains are distributed**.
:::
...
::: {.callout-tip}
**Good luck on Friday — and thank you again.** It's been a privilege to teach this cohort.
:::
